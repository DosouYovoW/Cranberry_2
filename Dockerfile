...
RUN apt-get update && apt-get install -y --no-install-recommends \
    libxml2-dev libcurl4-openssl-dev libssl-dev libudunits2-dev libv8-dev \
    && rm -rf /var/lib/apt/lists/*

# Install R packages (r2u provides fast precompiled binaries)
RUN Rscript -e "install.packages(c('plumber','jsonlite','tidyverse','tidymodels','bundle','xgboost','compositions','lubridate','recipes','readr','janitor','caret', 'kernlab'), repos='${CRAN_REPO}')"
